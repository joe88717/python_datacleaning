...
def read_data(connection):
    ...
    cursor.execute("SELECT SNO, ADDR FROM NCRM_STAGE_CIF_ADDR_API WHERE ADDR <> '' AND ADDR_API = '' ORDER BY 1")
    data = cursor.fetchall()
    return data

def update_data(connection, updates):
    cursor = connection.cursor()
    for sno, addr_api in updates:
        cursor.execute("UPDATE NCRM_STAGE_CIF_ADDR_API SET ADDR_API = ? WHERE SNO = ?", (addr_api, sno))
        print(f"更新資料：SNO={sno}, 新地址={addr_api}")
    connection.commit()
...
def num_to_chinese(num_str):
    ...
    if num == 10:
        return unit_map[10]
    if num < 20:
        return sno,unit_map[10] + digit_map[str(num % 10)]
...
def main():
    ...
    for i in range(0, len(data), batch_size):
        ...
        new_addrs = [(sno, convert_address(addr)) for sno, addr in batch]

        if new_addrs:
            if len(new_addrs) != len(batch):
                print(f"批次 {i // batch_size + 1} 回應數量不匹配，預期 {len(batch)} 筆，實際 {len(new_addrs)} 筆")
            
            updates = [(sno, addr_api if addr_api.strip() else None) for sno, addr_api in new_addrs]
