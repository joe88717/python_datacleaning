...
def read_data(connection):
    ...
    cursor.execute("SELECT SNO, ADDR FROM NCRM_STAGE_CIF_ADDR_API WHERE ADDR <> '' AND ADDR_PYTHON = '' ORDER BY 1")
...
def update_data(connection, updates):
    ...
    for sno, addr_python in updates:
        ...
        print(f"更新資料：SNO={sno}, 新地址={addr_python}")
    connection.commit()
...
def main():
    ...
    for i in range(0, len(data), batch_size):
        ...
        new_addrs = [(sno, convert_address(addr) if convert_address(addr).strip() else addr)for sno, addr in batch if addr is not None]

        if new_addrs:
            if len(new_addrs) != len(batch):
                print(f"批次 {i // batch_size + 1} 回應數量不匹配，預期 {len(batch)} 筆，實際 {len(new_addrs)} 筆")
            #20250829，因為資料表不允許null ，回傳none的話會失敗，多判斷 if addr_python is not None
            updates = [(sno, addr_python if addr_python.strip() else addr) for sno, addr_python in new_addrs if addr_python is not None]
            print(f"更新資料：{updates}")
        
        #  確保每一批都執行更新
            update_data(connection, updates)
            processed_records += len(batch)
            print(f"總共處理 {processed_records} 筆資料")
        else:
